...
# 🌐 DataNode UI: http://localhost:9864 (if running dedicated DataNode container)
```
...
## 🧩 Troubleshooting: DataNode not running

If the NameNode UI shows 0 live DataNodes or `jps` inside the Hadoop container does not list `DataNode`:

- Ensure Java home is correct (inside containers we use Eclipse Temurin):
  - JAVA_HOME should be `/opt/java/openjdk`.
  - We’ve set this in `hadoop/config/hadoop-env.sh` and docker-compose.

- Start or restart the services:
  - docker compose up -d hadoop
  - docker compose up -d hadoop-datanode-1    # optional additional DataNode

- Manually start inside the container (for debugging):
  - make shell-hadoop
  - service ssh start
...
- Check logs (examples):
  - tail -n 200 /opt/hadoop/logs/hadoop-*.log
  - tail -n 200 /opt/hadoop/logs/hadoop-*-datanode-*.log

## 🧱 Multi-node note

This project now includes an optional separate DataNode container `hadoop-datanode-1` in docker-compose:

- NameNode + one DataNode also run in the `hadoop` container for single-node scenarios.
- You can add more DataNodes by copying the `hadoop-datanode-1` service and changing:
  - service name, `hostname`, `container_name`, mapped port (9864 -> 9865, etc.), and volume name.
- After bringing them up, check NameNode UI → Datanodes shows them as Live.
